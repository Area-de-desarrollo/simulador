...
class VentilatorSimulator(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        # Controles comunes
        common_controls = [
            ("FR", self.resp_rate, 5, 40, self.set_resp_rate),
    ...
    def generate_next_point(self):
        ...
        insp_time = total_time * (self.ie_ratio / (1 + self.ie_ratio))
        exp_time = total_time - insp_time
    ...
    def update_graphs(self):
        ...
        self.volume_curve.setData(self.time_data, self.volume_data)
        
        # Ajustar rango del eje X para mostrar los últimos ciclos
        x_min = max(0, self.time_data[-1] - total_time * self.max_cycles)
        x_max = max(total_time * self.max_cycles, self.time_data[-1] + total_time * 0.1)
        
        for graph in [self.pressure_graph, self.flow_graph, self.volume_graph]:
            graph.setXRange(x_min, x_max, padding=0)
