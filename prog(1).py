...
class VentilatorSimulator(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.pressure_graph.setYRange(5,100)
        ...
        self.flow_graph.setYRange(-15,15)
        ...
        self.volume_graph.setYRange(2,2000)
    ...
    def generate_next_point(self):
        ...
        compliance = (self.tidal_volume/1000)/(self.peak_pressure - self.peep)
        ...
        if self.ventilation_mode == "VC-CMV":
            ...
            if cycle_progress * total_time < insp_time:
                ...
                #flow = 60 * (self.tidal_volume/1000) / insp_time * (1 - x)**2
                
                # Amortiguación más rápida: el exponente se multiplica por 2
                flow = 60 * (self.tidal_volume/1000) / insp_time * (1 - x)**2 * np.exp(-2 * x)
                
                
                #flow=((self.peak_pressure - self.peep) / self.resistance)*np.exp(((-1*x)/(self.resistance*compliance)))
               
                # #Flujo durante la inspiración (forma sinusoidal amortiguada)
                
                # zeta = 0.1  # Factor de amortiguamiento (entre 0 y 1, menor a 1 para oscilaciones)
                # omega_n = 8 # Frecuencia natural (ajusta según lo que quieras ver)
                # omega_d = omega_n * np.sqrt(1 - zeta**2)
                # A = (self.peak_pressure - self.peep) / self.resistance
                # flow = A * np.exp(-zeta * omega_n * x) * np.cos(omega_d * x)
                
                # #criticamente amortiguada
                
                # zeta = 0.5  # Críticamente amortiguado
                # omega_n = 5 # Frecuencia natural (ajusta según lo que quieras ver)
                # A = (self.peak_pressure - self.peep) / self.resistance
                # flow = A * x * np.exp(-omega_n * x)
                ...
                flow = -60 * (self.tidal_volume/1000) / exp_time * np.exp(-4*x)
                #flow=((-self.peak_pressure - self.peep) / self.resistance)*np.exp((-1*(x-insp_time))*(self.resistance*compliance))
               
                pressure = self.peep + (self.peak_pressure - self.peep) * np.exp(-4*x)
                # Volumen durante espiración (decaimiento exponencial)
                volume = self.tidal_volume * np.exp(-4*x)
